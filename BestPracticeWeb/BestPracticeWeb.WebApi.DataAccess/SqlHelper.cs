...
namespace BestPracticeWeb.WebApi.DataAccess
{
    /// <summary>
    /// SQL Server数据库访问助手类
    ...
    public class SqlHelper
    {
        ...
        public bool ExistsColumn(string table, string column)
        {
        ...
        /// <returns>是否存在</returns>
        public bool ColumnExistsValue(string table, string column, string value)
        {
        ...
        /// <returns>最大值</returns>
        public int QueryMaxId(string fieldName, string tableName)
        {
        ...
        public string GenerateQuerySql(string table, string[] columns, int index, int size, string where, string orderField, bool isDesc = true)
        {
            if (index == 1)
            {
                // 生成查询第一页SQL
        ...
        /// <param name="isDesc">排序方式(0:降序(desc)|1:升序(asc))(忽略则传入-1)</param>
        /// <returns>生成的T-SQL语句</returns>
        public string GenerateQuerySql(string table, string[] columns, string where, string orderField, bool isDesc = true)
        {
            // where语句组建
            where = string.IsNullOrEmpty(where) ? string.Empty : "where " + where;
            // 查询字段拼接
        ...
        /// <param name="isDesc">排序方式(0:降序(desc)|1:升序(asc))(忽略则传入-1)</param>
        /// <returns>生成的T-SQL语句</returns>
        public string GenerateQuerySql(string table, string[] columns, int size, string where, string orderField, bool isDesc = true)
        {
            // where语句组建
            where = string.IsNullOrEmpty(where) ? string.Empty : "where " + where;
            // 查询字段拼接
        ...
        /// <param name="parameters">参数列表</param>
        /// <exception cref="链接数据库异常"></exception>
        public void ExecuteReader(string cmdText, Action<SqlDataReader> action, params SqlParameter[] parameters)
        {
        ...
        /// <param name="parameters">参数列表</param>
        /// <exception cref="链接数据库异常"></exception>
        public void ExecuteReader(string cmdText, Action<SqlDataReader> action, CommandType type, params SqlParameter[] parameters)
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                using (SqlCommand cmd = new SqlCommand(cmdText, conn))
        ...
        /// <exception cref="链接数据库异常"></exception>
        /// <returns>SqlDataReader对象</returns>
        public SqlDataReader ExecuteReader(string cmdText, CommandType type, params SqlParameter[] parameters)
        {
        ...
        /// <param name="parameters">参数列表</param>
        /// <returns>离线数据集DataSet</returns>
        public DataSet ExecuteDataSet(string cmdText, CommandType type, params SqlParameter[] parameters)
        {
        ...
        /// <param name="parameters">参数列表</param>
        /// <returns>查询到的数据表</returns>
        public DataTable ExecuteDataTable(string cmdText, CommandType type, params SqlParameter[] parameters)
        {
        ...
        #region ExecuteNonQueryByTransaction +(string cmdText, CommandType type, params SqlParameter[] parameters)
        /// <summary>
        /// 执行sql （事务）
        /// </summary>
        ...
        /// <returns></returns>
        public int ExecuteNonQueryByTransaction(string cmdText, CommandType type, params SqlParameter[] parameters)
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                conn.Open();
        ...
        public int ExecuteNQByTransaction(string cmdText, params SqlParameter[] parameters)
        {
            return ExecuteNonQueryByTransaction(cmdText, CommandType.Text, parameters);
        }
    }
}
