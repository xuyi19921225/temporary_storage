...
namespace BestPracticeWeb.WebApi.Repository
{
    public class BaseRepository<TEntity>:IBaseRepository<TEntity> where TEntity:class,new()
    {
        ...
        {
            return await Task.Run(() => db.Queryable<TEntity>().With(SqlWith.NoLock).Where(whereExpression).First());
        }
        ...
        {
            IInsertable<TEntity> up = db.Insertable(entity);

            //写入操作 忽略更新字段
            string[] arrStr = new string[] { "UpdatedBy", "UpdatedAt" };
            up.IgnoreColumns(arrStr);

            var i = await Task.Run(() => up.ExecuteReturnBigIdentity());
        ...
        public async Task<bool> Update(TEntity entity)
        {
            ...
            //更新操作 忽略创建字段
            string[] arrStr = new string[] { "CreateBy", "CreateAt" };
            up.IgnoreColumns(arrStr);
            //这种方式会以主键为条件
        ...
        /// </summary>
        /// <param name="entity">entity</param>
        /// <param name="lstIgnoreColumns">lstIgnoreColumns</param>
        /// <returns></returns>
        public async Task<bool> Update(TEntity entity,
        ...
        public async Task<int> Add(List<TEntity> model,List<string> lsgIngoreColumns=null)
        {
            return await db.Insertable(model).IgnoreColumns(lsgIngoreColumns.ToArray()).ExecuteCommandAsync(); 
        }
    }
}
