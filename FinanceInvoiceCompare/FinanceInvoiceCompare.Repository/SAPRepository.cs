...
namespace FinanceInvoiceCompare.WebApi.Repository
{
    public class SAPRepository : BaseRepository<SAPInvoiceData>, ISAPRepository
    {
        ...
        public async Task<PageModel<SAPInvoiceData>> GetSAPInvoiceList(SAPRequestModel model)
        {
            ...
            {
                List = await Db.Queryable(allSapInvoice, groupSapInvoice,JoinType.Left, (p1, p2) => p1.Cocd == p2.Cocd && p1.Reference == p2.Reference)
                .WhereIF(!string.IsNullOrEmpty(model.IsMatch), (p1,p2) => p1.IsMatch==model.IsMatch)
                .WhereIF(model.Check!=null, (p1, p2) => p2.Check == model.Check)
        ...
        public async Task<List<SAPInvoiceData>> GetAllSAPInvoiceList(SAPRequestModel model)
        {
            var allSapInvoice = Db.Queryable<SAPInvoiceData, Vendor, Invoice>((a1, b2, c3) => new object[]
               {
                        JoinType.Left,a1.Vendor==b2.VendorCode&&a1.Cocd==b2.CompanyCode&&b2.IsDelete==false,
            ...
            return await Db.Queryable(allSapInvoice, groupSapInvoice, JoinType.Left, (p1, p2) => p1.Cocd == p2.Cocd && p1.Reference == p2.Reference)
                .WhereIF(!string.IsNullOrEmpty(model.IsMatch), (p1, p2) => p1.IsMatch == model.IsMatch)
                .WhereIF(model.Check != null, (p1, p2) => p2.Check == model.Check)
                .Select((p1, p2) => new SAPInvoiceData
                {
                ...
                .OrderBy((p1) => p1.Id, OrderByType.Desc).ToListAsync();
