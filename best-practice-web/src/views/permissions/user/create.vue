...
import { getSAPUserInfoByWorkdayID, AddUser } from '@/api/user'
...
export default {
  ...
  methods: {
    getSiteList() {
      getSiteAreaListByParameter({ isActive: 0, parentID: -1 }).then(res => {
        this.siteList = res.response
      })
    },
    getDivisonListBySiteID() {
      getSiteAreaListByParameter({ isActive: 0, parentID: this.form.siteID }).then(res => {
        this.divisionList = res.response
      })
    },
    getSAPUserInfoByWorkdayID() {
      getSAPUserInfoByWorkdayID({ workdayID: this.form.jobNumber }).then(res => {
        if (res.response) {
